...
const DesignPreview = ({ configuration }: { configuration: Configuration }) => {
  ...
  const {mutate: createPaymentSession} = useMutation({
    ...
    onSuccess: ({url}) => {
      if(url){
        ...
        throw new Error("Unable to retireve payment URL")
      }
    },
    onError: ()=> {
      toast({
  ...
  const handleCheckout = () => {
    if(user){
      ...
      // need to login
      localStorage.setItem("PHONECASE_CONFIGID", configuration.id)
        ...
        <div className="sm:col-span-12 md:col-span-9 text-base">
          ...
          <div className="mt-8">
            ...
            <div className="mt-8 flex justify-end pb-12">
                <Button className="px-4 sm:px-6 lg:px-8" onClick={handleCheckout}>Check out <ArrowRight className="h-4 w-4 ml-1.5 inline"/></Button>
